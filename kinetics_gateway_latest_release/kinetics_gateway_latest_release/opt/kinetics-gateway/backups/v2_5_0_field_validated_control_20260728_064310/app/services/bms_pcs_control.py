...
class BmsPcsControlService:
    ...
    def status(self, pair_id: str, *, fresh: bool = True) -> dict[str, Any]:
        ...
        # PCS protocol 0x1202 bit 6: 0=DC breaker closed, 1=DC breaker open.
        pcs_dc_breaker_open = (
            None if pcs_status_word_2_raw is None else bool(pcs_status_word_2_raw & (1 << 6))
        )
        # PCS protocol 0x1210 bit 7: 0=DC breaker feedback open, 1=closed.
    ...
    def prepare_pcs_standby(self, username: str, pair_id: str, confirmation: str) -> dict[str, Any]:
        """Configure and start the PCS into zero-power standby before BMS precharge.

        The BMS rack must remain disabled and all BMS contactors open. This stage
        only writes PCS Remote/PQ/Constant-Power/0-kW settings and the PCS Start
        command, then requires Standby + Remote + Authorization feedback.
        """
        pair = self._pair(pair_id)
        self._require_write_gates(confirmation)
        ...
        pcs_state = summary.get("pcs_operating_state")
        pcs_dc_bus = summary.get("pcs_dc_bus_voltage_v")
        pcs_battery_voltage = summary.get("pcs_battery_voltage_v")
        pcs_setpoint = summary.get("pcs_power_setpoint_kw")
        pcs_actual = summary.get("pcs_actual_power_kw")
        precharge_command = summary.get("precharge_command")
        precharge_state = summary.get("precharge_state")
        safe_checks = {
            ...
            "pcs_setpoint_zero": pcs_setpoint is not None and abs(float(pcs_setpoint)) <= PCS_ZERO_SETPOINT_TOLERANCE_KW,
            "pcs_actual_power_zero": pcs_actual is not None and abs(float(pcs_actual)) <= PCS_ZERO_ACTUAL_POWER_TOLERANCE_KW,
            "pcs_dc_bus_deenergized": pcs_dc_bus is not None and abs(float(pcs_dc_bus)) <= PCS_PRECHARGE_PREP_MAX_DEENERGIZED_V,
            "pcs_battery_input_deenergized": (
                pcs_battery_voltage is not None
                and abs(float(pcs_battery_voltage)) <= PCS_PRECHARGE_PREP_MAX_DEENERGIZED_V
            ),
        }
        if not all(safe_checks.values()):
            failed = [key for key, passed in safe_checks.items() if not passed]
            raise ValueError(f"PCS standby preparation safety precheck failed: {failed}")
        ...
        try:
            ...
            writes.append(
                self.pcs_driver.write_point(
                    pair.pcs_asset_id, "remote_on_off_command", PCS_START_VALUE
                )
            )
            ...
            while time.monotonic() < deadline:
                ...
                current_state = int(current_summary.get("pcs_operating_state") or 0)
                sample = {
                    "sample": len(samples) + 1,
                    "pcs_operating_state": current_state,
                    "pcs_remote_feedback": current_summary.get("pcs_remote_feedback"),
                    "pcs_authorized": current_summary.get("pcs_authorized"),
                    "pcs_power_setpoint_kw": current_summary.get("pcs_power_setpoint_kw"),
                    "pcs_actual_power_kw": current_summary.get("pcs_actual_power_kw"),
                    "pcs_dc_bus_voltage_v": current_summary.get("pcs_dc_bus_voltage_v"),
                    "pcs_battery_voltage_v": current_summary.get("pcs_battery_voltage_v"),
                    "pcs_dc_breaker_open": current_summary.get("pcs_dc_breaker_open"),
                ...
                if current_state & PCS_FAULT_SHUTDOWN_BIT:
                    raise RuntimeError(f"PCS entered fault shutdown during standby preparation: {current_state}")
                if bool(sample["pcs_dc_soft_start_fault"]) or bool(sample["pcs_dc_soft_start_relay_fault"]):
                    raise RuntimeError("PCS reported a DC soft-start/relay fault during standby preparation")
                actual = sample["pcs_actual_power_kw"]
                if actual is None or abs(float(actual)) > PCS_ZERO_ACTUAL_POWER_TOLERANCE_KW:
                    raise RuntimeError(f"PCS actual power was not zero during standby preparation: {actual}")
                if current_state not in PCS_PRECHARGE_PREP_ALLOWED_STATES:
                    raise RuntimeError(
                        f"PCS entered unexpected state {current_state}; expected Stop/SoftStart/SelfTest/Standby"
                    )
                if current_state == PCS_STANDBY_STATE:
                    remote_ok = sample["pcs_remote_feedback"] is True
                    authorization_ok = sample["pcs_authorized"] is True
                    ready_for_bms_precharge = remote_ok and authorization_ok
                    response = {
                        "ok": ready_for_bms_precharge,
                        "stage": (
                            "pcs_standby_ready_for_bms_precharge"
                            if ready_for_bms_precharge
                            else "pcs_standby_feedback_incomplete"
                        ),
                        "safe_checks": safe_checks,
                        "writes": writes,
                        "samples": samples,
                        "ready_for_bms_precharge": ready_for_bms_precharge,
                        "required_feedback": {
                            "operating_state": PCS_STANDBY_STATE,
                            "remote_feedback": True,
                            "authorization": True,
                            "power_setpoint_kw": 0.0,
                            "actual_power_kw": 0.0,
                        },
                        "next_action": (
                            "Enable Rack 1, then issue BMS precharge 0x0402=1"
                            if ready_for_bms_precharge
                            else "Do not precharge; confirm PCS remote/authorization feedback with PCS vendor"
                        ),
                    }
                    self._update_runtime(
                        pair_id,
                        stage=response["stage"],
                        last_action="prepare_pcs_standby",
                        last_error=None if ready_for_bms_precharge else "PCS standby feedback incomplete",
                        commanded_power_kw=0.0,
                    )
                    self._audit(
                        username,
                        ...
                        request_payload,
                        response,
                        "success" if ready_for_bms_precharge else "failed",
                    )
                    return response
                time.sleep(self.config.sample_interval_seconds)

            raise TimeoutError("PCS did not reach Standby state before the configured timeout")
        except Exception as error:
            for key, value in (
                ("active_power_setpoint", 0.0),
            ...
            failure = {
                "ok": False,
                "stage": "pcs_standby_preparation_failed",
                "error": str(error),
                "safe_checks": safe_checks,
                "writes": writes,
                "samples": samples,
                "automatic_pcs_safe_stop": safety_actions,
            }
            self._update_runtime(
                pair_id,
                stage="error",
            ...
            self._audit(username, pair, "prepare_pcs_standby", request_payload, failure, "failed")
    ...
    def capabilities(self) -> dict[str, Any]:
        return {
            ...
            "pcs_registers": {
                ...
                "operating_state": "0x1200: 1=Stop, 2=SoftStart, 4=SelfTest, 8=Standby",
