...
async def amain()->None:
    ...
    container=DependencyContainer.create(config=config,register_map=reg); reader=build_reader(config,args.mock); scheduler=PollingScheduler(container,reader); uploader=ServerUploadService(config.server_upload,container); container.server_upload_service=uploader
    print('NorthBound EMS Gateway'); print('  version: 0.5.0'); print(f'  mode: {config.gateway.mode}'); print(f'  mock: {args.mock}'); print(f'  EMS: {config.existing_ems.host}:{config.existing_ems.port}, unit={config.existing_ems.unit_id}'); print(f'  register points: {reg.point_count}'); print(f'  API: http://{config.api.host}:{config.api.port}'); print(f'  logs API: enabled={config.logs_api.enabled}, http://{config.logs_api.host}:{config.logs_api.port}'); print(f'  storage: {config.storage.path}'); print(f'  storage required mount: {config.storage.required_mount_path}'); print(f'  server upload: enabled={config.server_upload.enabled}, interface={config.server_upload.network_interface}')
    try:
