...
class BmsPcsControlService:
    ...
    def _workflow_from_summary(self, summary: dict[str, Any], runtime: dict[str, Any]) -> dict[str, Any]:
        ...
        steps = [
            ...
            {"key": "rack_enabled", "label": "Rack enabled", "complete": rack_enabled},
    ...
    def prepare_pcs_standby(self, username: str, pair_id: str, confirmation: str) -> dict[str, Any]:
        ...
        enable_rack(), start_precharge(), and start_pcs().
        """
        self._pair(pair_id)
        self._require_write_gates(confirmation)
        raise ValueError(
    ...
    def configure_pcs(self, username: str, pair_id: str, confirmation: str) -> dict[str, Any]:
        ...
        Rack 1 is enabled, BMS precharge closes the main contactors, and only
    ...
    def _automatic_step_template() -> list[dict[str, Any]]:
        return [
            ...
            {"key": "rack_enable", "label": "Enable BMS rack", "status": "pending"},
        ...
        *,
        ...
        try:
            def stopped(item: dict[str, Any]) -> bool:
                ...
                return (
                    base
                    and summary.get("rack_enabled") == 0
                    and int(summary.get("precharge_state") or 0) == 0
                    and not bool(summary.get("positive_contactor_closed"))
                    and not bool(summary.get("negative_contactor_closed"))
                )
        ...
        response = {
            ...
            "residual_voltage_note": (
                ...
                "zero power, PCS stopped, rack disabled and contactors open."
            ),
        }
        self._update_runtime(
            pair_id,
    ...
    def automatic_start(
        ...
        *,
        ...
        response = {
            ...
            "status_endpoint": f"/api/control-sequence/{pair_id}/status?fresh=true",
        }
    ...
    def _automatic_worker(
        ...
        try:
            ...
            result = self.enable_rack(
                username,
                pair_id,
                direction,
                max(target_power_kw, 1.0),
                self.config.confirmation_phrase,
            )
            status = self._wait_for_status(
                pair_id,
                lambda item: item["summary"].get("rack_enabled") == 1,
                timeout_seconds=self.config.automatic_stage_timeout_seconds,
                description="rack enable feedback",
                abort_event=abort_event,
            )
            self._set_run_step(pair_id, "rack_enable", "success", result={"write": result, "status": status})
    ...
    def next_step(
        ...
        if not (
            ...
            and abs(float(summary.get("pcs_power_setpoint_kw") or 0.0)) <= PCS_ZERO_SETPOINT_TOLERANCE_KW
        ) and summary.get("rack_enabled") != 1:
            action = "configure_pcs"
            result = self.configure_pcs(username, pair_id, confirmation)
        elif summary.get("rack_enabled") != 1:
            action = "enable_rack"
            result = self.enable_rack(username, pair_id, direction, max(float(target_power_kw), 1.0), confirmation)
        elif not bool(summary.get("precharge_success")) or not bool(summary.get("contactors_ready")):
            action = "start_precharge"
            write = self.start_precharge(username, pair_id, confirmation)
            verified = self._wait_for_status(
                pair_id,
                lambda item: bool(item["summary"].get("precharge_success")) and bool(item["summary"].get("contactors_ready")),
                timeout_seconds=self.config.automatic_stage_timeout_seconds,
                description="BMS precharge success",
            )
            result = {"write": write, "verified": verified}
        elif not bool(workflow.get("ready_for_power")):
            action = "start_pcs"
            ...
            if abs(current_signed - target_signed) > 0.2:
                action = "set_power"
                result = self.set_power(username, pair_id, direction, float(target_power_kw), confirmation)
            else:
                action = "complete"
                result = {"ok": True, "stage": "requested_state_already_reached", "status": status}
        return {"ok": True, "action": action, "result": result, "status": self.status(pair_id, fresh=True)}
    ...
    def _runtime_monitor_worker(self, username: str, pair: ControlPairConfig) -> None:
        ...
        try:
            while True:
                status = self.status(pair_id, fresh=True)
                summary = status["summary"]
                commanded = float(summary.get("pcs_power_setpoint_kw") or 0.0)
                if abs(commanded) <= PCS_ZERO_SETPOINT_TOLERANCE_KW:
                    break
                ...
                if prohibited:
                    violations.append(f"{direction}_prohibited")
                if dynamic_limit is None or abs(commanded) > float(dynamic_limit) + 1e-9:
                ...
                if violations:
                    ...
                    return
                time.sleep(self.config.runtime_monitor_interval_seconds)
        except Exception as error:
            ...
            self._update_runtime(pair_id, monitor_active=False)
    ...
    def capabilities(self) -> dict[str, Any]:
        return {
            ...
            "api_endpoints": {
                "status": "GET /api/control-sequence/{pair_id}/status?fresh=true",
