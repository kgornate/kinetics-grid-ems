...
def build_router(service: GatewayService, auth: AuthService) -> APIRouter:
    ...
    def diagnostics_control_refresh(
        ...
        return service.control_sequence.refresh_diagnostics()

    @router.get("/api/diagnostics/runtime-monitor")
    def diagnostics_runtime_monitor(
        user: User = Depends(current_user),
    ) -> dict[str, Any]:
        return service.control_sequence.runtime_monitor_diagnostics()

    @router.get("/api/diagnostics/runtime-monitor/{pair_id}/sample")
    def diagnostics_runtime_monitor_sample(
        pair_id: str,
        user: User = Depends(current_user),
    ) -> dict[str, Any]:
        try:
            return service.control_sequence.runtime_monitor_sample(pair_id)
    ...
    @router.get("/api/control-sequence/status/all")
    def control_sequence_all_status(
        user: User = Depends(current_user),
    ) -> dict[str, Any]:
        return service.control_sequence.all_pair_status()

    @router.get("/api/control-sequence/status/all/compact")
    def control_sequence_all_status_compact(
        user: User = Depends(current_user),
    ) -> dict[str, Any]:
        return service.control_sequence.all_pair_status_compact()

    @router.post("/api/control-sequence/safe-stop-all")
    def control_sequence_safe_stop_all(
        request: SequenceStopRequest,
        user: User = Depends(require_internal),
    ) -> dict[str, Any]:
        try:
            return service.control_sequence.safe_stop_all(
                user.username, request.confirmation, open_bms=request.open_bms
            )
    ...
    def control_sequence_status(
        ...
        try:
            return service.control_sequence.status(
                pair_id,
                fresh=fresh,
                # Public live refreshes are engineering diagnostics. Never let
                # repeated HTTP polling queue behind fieldbus activity.
                wait_for_refresh=not fresh,
                return_cached_if_busy=fresh,
            )
        except ControlStatusBusyError as error:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail={
                    "code": "live_status_busy",
                    "message": str(error),
                    "pair_id": pair_id,
                    "normal_polling": f"/api/control-sequence/{pair_id}/status?fresh=false",
                },
            ) from error
        except KeyError as error:
            raise HTTPException(status_code=404, detail=str(error)) from error
