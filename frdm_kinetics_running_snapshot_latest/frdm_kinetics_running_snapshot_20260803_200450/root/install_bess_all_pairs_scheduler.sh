...
SERVICE="bess-all-pairs-scheduler.service"
APP="/root/bess_all_pairs_scheduler.py"
ENV_FILE="/etc/bess-all-pairs-scheduler.env"
...
if [ ! -x /opt/kinetics-gateway/venv/bin/python ]; then
    echo "ERROR: gateway Python environment is missing"
    exit 1
fi

if [ ! -f /opt/kinetics-gateway/backend/tools/control_sequence_cli.py ]; then
    echo "ERROR: control_sequence_cli.py is missing"
    exit 1
fi

systemctl stop "$SERVICE" 2>/dev/null || true

for file in "$APP" "$ENV_FILE" "$UNIT_FILE"; do
    if [ -e "$file" ]; then
        cp -a "$file" "$file.backup_$STAMP"
    fi
done

cat > "$APP" <<'PYTHON_SCRIPT'
#!/usr/bin/env python3
...
cat > "$ENV_FILE" <<'ENVIRONMENT_FILE'
API_USER=internal
API_PASS=Internal@123

BASE_URL=http://127.0.0.1:8000
BMS_HOST=192.168.111.22
BMS_SOURCE_IP=192.168.111.2
BAU_UNIT_ID=1
SOC_ADDRESS=0x2004
BAU_CONNECT_ADDRESS=0x3001

# Stop each individual pair as soon as its own SOC reaches 96%.
CHARGE_STOP_SOC=96.0

# During discharge, safely stop each individual pair at or below 20% SOC.
MIN_DISCHARGE_SOC=20.0

# Positive power means discharge on this PCS mapping.
DISCHARGE_KW=200.0

# Four pairs x 200 kW = 800 kW, below the confirmed 1000 kW site limit.
SITE_MAX_TOTAL_DISCHARGE_KW=1000.0

# Start discharge 2 hours 15 minutes after this service is first started.
START_DELAY_MINUTES=135

# Begin safe-stop/precharge/PCS preparation 20 minutes before discharge time.
PREPARE_LEAD_MINUTES=20

# Stop all operating pairs 2 hours after the discharge commands are issued.
DISCHARGE_DURATION_MINUTES=120

SOC_POLL_SECONDS=20
DISCHARGE_POLL_SECONDS=10

ARM_PHRASE=EXECUTE_ALL_4_PAIR_SCHEDULE
ENVIRONMENT_FILE
...
[Unit]
Description=BESS Four-Pair SOC Cutoff and Scheduled Discharge Supervisor
After=network-online.target kinetics-gateway.service
Requires=kinetics-gateway.service

[Service]
Type=simple
WorkingDirectory=/opt/kinetics-gateway/backend
EnvironmentFile=/etc/bess-all-pairs-scheduler.env
Environment=PYTHONUNBUFFERED=1
ExecStart=/usr/bin/python3 /root/bess_all_pairs_scheduler.py
Restart=no
KillSignal=SIGTERM
TimeoutStopSec=900
...
echo
echo "Start the schedule with:"
echo "  systemctl start $SERVICE"
echo
echo "Follow live logs with:"
echo "  journalctl -fu $SERVICE"
echo
echo "The 2h15m countdown begins when the service starts."
