...
def main() -> int:
    ...
    parser.add_argument("command", choices=[
        ...
        "prepare-pcs-standby", "configure-pcs", "start-pcs", "set-power",
        "verify-power", "zero-power", "safe-stop", "automatic-start",
        "next-step", "abort",
    ])
    ...
    parser.add_argument("--ramp-step-kw", type=float)
    ...
    if args.command == "capabilities":
    ...
    elif args.command in {"start-insulation", "start-precharge", "recover-bms", "prepare-pcs-standby", "configure-pcs", "start-pcs"}:
        endpoint = {
            "start-insulation": "start-insulation",
            "start-precharge": "start-precharge",
            "recover-bms": "recover-bms",
            "prepare-pcs-standby": "prepare-pcs-standby",
    ...
    elif args.command == "zero-power":
        result = request_json(
            f"{base}/api/control-sequence/{pair}/zero-power",
            method="POST", token=token,
            payload={"confirmation": args.confirmation},
        )
    elif args.command == "automatic-start":
        if args.direction is None or args.power_kw is None:
            parser.error("automatic-start requires --direction and --power-kw")
        payload = {
            "direction": args.direction,
            "power_kw": args.power_kw,
            "confirmation": args.confirmation,
        }
        if args.ramp_step_kw is not None:
            payload["ramp_step_kw"] = args.ramp_step_kw
        if args.ramp_interval_seconds is not None:
            payload["ramp_interval_seconds"] = args.ramp_interval_seconds
        result = request_json(
            f"{base}/api/control-sequence/{pair}/automatic-start",
            method="POST", token=token, payload=payload,
        )
    elif args.command == "next-step":
        if args.direction is None or args.power_kw is None:
            parser.error("next-step requires --direction and --power-kw")
        result = request_json(
            f"{base}/api/control-sequence/{pair}/next-step",
            method="POST", token=token,
            payload={
                "direction": args.direction,
                "power_kw": args.power_kw,
        ...
        result = request_json(
            f"{base}/api/control-sequence/{pair}/abort",
            method="POST", token=token,
            payload={"confirmation": args.confirmation, "open_bms": args.open_bms},
        )
    else:
