...
class BmsModbusDriver:
    ...
    def _build_endpoints(self) -> dict[str, Endpoint]:
        endpoints = {
            # Legacy bank alias retained for existing API clients. In separate-port
            # deployments it represents the BAU reachable through the configured
            # primary BAU endpoint (normally Pair 1 / port 503).
        ...
        for rack in self.config.racks:
            # Every BMS TCP port exposes the same BAU register map at Unit ID 1
            # and the corresponding BCU register map at Unit ID 2. Expose a
            # pair-specific bank asset so control writes never fall through to
            # the legacy Pair 1 BAU endpoint.
