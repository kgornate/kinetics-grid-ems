...
SCHEDULER_BUILD = "v302-lean-sequential-20260804-r4-compact-status-hotfix"
...
COMPACT_STATUS_CACHE_SECONDS = float(
    os.environ.get("COMPACT_STATUS_CACHE_SECONDS", "0.5")
)
ALLOW_AUTOMATIC_START_FALLBACK = os.environ.get(
    "ALLOW_AUTOMATIC_START_FALLBACK", "false"
).strip().lower() in {"1", "true", "yes", "on"}
ACTIVE_WINDOW_CATCHUP_MARKER = Path(
    os.environ.get(
        "ACTIVE_WINDOW_CATCHUP_MARKER",
        "/run/bess-seven-day-v302.allow-active-start-once",
    )
)
...
class ApiClient:
    def __init__(self) -> None:
        ...
        self._compact_status_cache: dict[str, dict[str, Any]] = {}
        self._compact_status_cache_time: float = 0.0
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                with urllib.request.urlopen(request, timeout=timeout) as response:
                    if discard_response_body:
                        return {
                            "ok": True,
                            "_http_status": int(response.getcode()),
                            "_body_discarded": True,
                        }
                    body = response.read(MAX_API_BODY_BYTES + 1)
                if len(body) > MAX_API_BODY_BYTES:
    ...
    def _refresh_compact_status_cache(self, *, force: bool = False) -> None:
        now = time.monotonic()
        if (
            not force
            and self._compact_status_cache
            and now - self._compact_status_cache_time
            <= COMPACT_STATUS_CACHE_SECONDS
        ):
            return
        result = self.request(
            "GET",
        ...
        by_pair = result.get("by_pair_id") or {}
        if not isinstance(by_pair, dict):
            raise RuntimeError("compact status response omitted by_pair_id")
        missing = [pair_id for pair_id in PAIR_IDS if pair_id not in by_pair]
        if missing:
            raise RuntimeError(
                f"compact status response omitted pairs: {missing}"
            )
        self._compact_status_cache = {
            str(pair_id): dict(value or {})
            for pair_id, value in by_pair.items()
        }
        self._compact_status_cache_time = now
    ...
    def pair_status(
        ...
        *,
        ...
        # The pair endpoint includes the complete automatic-run history. After a
        # long or failed run that payload can grow to multiple megabytes and
        # block the scheduler for minutes. The backend already provides a small,
        # cache-only endpoint specifically for routine control/status polling.
        self._refresh_compact_status_cache(force=fresh)
        status = dict(self._compact_status_cache.get(pair_id) or {})
        if not status:
            raise RuntimeError(f"compact status is unavailable for {pair_id}")
        ...
        fault = any(
            (
                ...
                bool(workflow.get("hard_blocked")),
            )
        )
        soc = None
        if include_soc:
...
def ready_zero(item: PairStatus, direction: str) -> bool:
    return (
        # 0x20 (32) is the vendor energy-saving/ready state. It was observed in
        # the earlier successful field run immediately before set-power changed
        # the PCS to grid-connected state 0x50 (80).
        item.pcs_state in {16, 32, 80}
        and bms_connected(item)
        and direction_allowed(item, direction)
        and power_zero(item)
        and not item.errors
        and item.ready_for_power
    )
...
def post_pair(
    ...
    *,
    ...
    return CLIENT.request(
        ...
        # Successful control routes are verified through compact status. Do not
        # download their verbose nested status/audit response, which can exceed
        # several megabytes after a long automatic run.
        ...
        for attempt in (1, 2):
            try:
                ...
                if can_retry:
                    ...
                    continue
                break
    log(f"P{PAIR_NUMBER[pair_id]}: staged start failed ({last_error})")

    if not ALLOW_AUTOMATIC_START_FALLBACK:
        log(
            f"P{PAIR_NUMBER[pair_id]}: automatic-start fallback is disabled; "
            "returning pair to a verified safe baseline"
        )
        safe_stop_pair(pair_id, f"direct {direction} set-power failure")
        return False
...
def run_session(day: date, direction: str, state: dict[str, Any]) -> None:
    ...
    # Restart within an active window normally adopts already-active pairs only.
    # A root-created one-shot marker permits a supervised catch-up start after a
    # scheduler repair. The marker is consumed before any write, so a later
    # reboot cannot unexpectedly start stopped pairs during an active window.
    if now >= times.start:
        allow_catchup = ACTIVE_WINDOW_CATCHUP_MARKER.exists()
        if allow_catchup:
            try:
                ACTIVE_WINDOW_CATCHUP_MARKER.unlink()
            except FileNotFoundError:
                allow_catchup = False
        mode = "one-shot catch-up" if allow_catchup else "adopt-only"
        log(f"Scheduler entered during active {direction} window; {mode} mode")
        ...
        for pair_id, item in snapshots.items():
            if pair_id in active:
                continue
            if not allow_catchup and not fully_stopped(item):
        ...
        if allow_catchup:
            ...
            if datetime.now(IST) >= latest_start:
                log(
                    f"One-shot {direction} catch-up rejected because less than "
                ...
                for pair_id in PAIR_IDS:
                    ...
                    if pair_id in active:
                        continue
                    current = CLIENT.pair_status(pair_id, include_soc=True, fresh=True)
                    if not soc_allows(current, direction):
                        log(
                            f"P{PAIR_NUMBER[pair_id]}: excluded from one-shot "
                            f"{direction} catch-up; SOC={current.soc}"
                        )
                        continue
                    if not fully_stopped(current) and not ready_zero(current, direction):
                        if not safe_stop_pair(
                            pair_id,
                            f"one-shot {direction} catch-up baseline",
                        ):
                            continue
                    latest = CLIENT.pair_status(pair_id, include_soc=False, fresh=True)
                    if not ready_zero(latest, direction):
                        if not prepare_pair(pair_id, direction):
                            continue
                    ...
                    time.sleep(PAIR_COMMAND_SPACING_SECONDS)
        if active:
            log(f"Active {direction} pairs after reconciliation: {sorted(active)}")
