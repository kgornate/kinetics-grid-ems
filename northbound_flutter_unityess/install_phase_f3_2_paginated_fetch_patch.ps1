...
Write-Host "Applying Phase F3.2 Historian paginated fetch patch to: $ProjectPath"

$backup = Join-Path $ProjectPath ("backup_before_phase_f3_2_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
$files = @(
  ...
  "PHASE_F3_2_PAGINATED_FETCH_NOTES.md"
)
...
if ($content -notmatch "Load full range for export") { throw "Verification failed: paginated full-range button not found." }
if ($content -notmatch "Custom date/time") { throw "Verification failed: custom date/time filter not found." }
if ($content -notmatch "Rows/page") { throw "Verification failed: table pagination rows/page control not found." }
if ($content -notmatch "Download CSV") { throw "Verification failed: Download CSV not found." }
if ($content -notmatch "Download Excel") { throw "Verification failed: Download Excel not found." }
