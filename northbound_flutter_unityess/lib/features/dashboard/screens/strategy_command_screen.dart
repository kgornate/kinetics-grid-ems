...
class _StrategyCommandScreenState extends State<StrategyCommandScreen> {
    ...
    try {
      ...
      for (final source in sourceItems) {
        ...
        for (final asset in assetItems) {
          ...
          final baseAsset = asset['base_asset_id']?.toString().toLowerCase() ?? '';
          ...
          if (baseAsset == 'ems_system' || assetId.contains('ems_system') || assetId.contains('existing_ems')) {
            ...
            emsTelemetry = await api.getAssetTelemetry(assetIdValue, compact: false, keyOnly: false, pageSize: 4000);
            break;
          }
        }
  ...
  Widget build(BuildContext context) {
    final selected = _snapshots.where((e) => e.sourceId == _selectedSourceId).cast<EmsSystemSourceSnapshot?>().firstWhere(
      (e) => e != null,
      orElse: () => _snapshots.isNotEmpty ? _snapshots.first : null,
    );
        ...
        child: ListView(
          ...
          children: [
            if (_bootLoading && selected == null)
              ...
              Text('Strategy / Command Panel', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
              const SizedBox(height: 10),
              Text('Readback-oriented engineering page for EMS control modes, strategy states, cutoff thresholds, and command register values.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: const Color(0xFF6C7B8A))),
              ...
              if (selected == null)
                const Card(child: Padding(padding: EdgeInsets.all(18), child: Text('No strategy / command data available.')))
              else ...[
                Card(
                  child: Padding(
                    padding: const EdgeInsets.all(18),
                    child: Row(
                      children: [
                        ...
                        Container(
                          ...
                          child: const Text('Write actions intentionally hidden in this build', style: TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF6C7B8A))),
                        ),
                      ],
                    ),
                  ),
                ...
                GridView.count(
                  ...
                  children: [
                    ...
                    HomeKpiTile(title: 'Manual Mode', value: selected.manualModeControlLabel, subtitle: 'Manual mode readback', icon: Icons.handyman_rounded),
                    HomeKpiTile(title: 'Auto Mode', value: selected.autoModeControlLabel, subtitle: 'Automatic strategy readback', icon: Icons.auto_mode_rounded),
                    HomeKpiTile(title: 'Charge / Discharge Control', value: selected.chargeDischargeControlModeLabel, subtitle: 'EMS charge-discharge mode', icon: Icons.battery_charging_full_rounded),
                    ...
                    HomeKpiTile(title: 'PCS Power Control', value: selected.pcsPowerControlLabel, subtitle: 'PCS on/off state', icon: Icons.electrical_services_rounded),
                    HomeKpiTile(title: 'BMS Power Control', value: selected.bmsPowerControlLabel, subtitle: 'BMS on/off state', icon: Icons.battery_full_rounded),
                ...
                Card(
                  child: Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Strategy Thresholds / Configured Limits', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
                        const SizedBox(height: 12),
                        if (selected.configItems.isEmpty)
                          const Text('No configuration / threshold fields exposed.')
                        else
                          ...selected.configItems.take(28).map((item) => _valueRow(item.displayName, item.stateLabel)),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _commandReadbackCard(BuildContext context, EmsSystemSourceSnapshot snapshot) {
    final commandish = snapshot.configItems.where((item) {
      final key = item.signalName.toLowerCase();
      return key.contains('control') || key.contains('command') || key.contains('mode') || key.contains('power_on');
    }).take(24).toList();
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Command Register Readback', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: 12),
            if (commandish.isEmpty)
              const Text('No command/readback fields exposed for this source.')
            else
              ...commandish.map((item) => _valueRow(item.displayName, item.stateLabel)),
          ],
        ),
      ),
    );
  ...
  Widget _valueRow(String label, String value) => Container(
        ...
        child: Row(
          children: [
            ...
            Text(value, style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF5B6775))),
          ],
        ),
      );
