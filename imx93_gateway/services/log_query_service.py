...
class LogQueryService:
    ...
    def telemetry_dir(self, asset_id: Optional[Any] = None) -> Path:
        return self.base_path / "logs" / self._resolve_asset_id(asset_id)

    def events_file(self, asset_id: Optional[Any] = None) -> Path:
        asset = self._resolve_asset_id(asset_id)
        return self.base_path / "events" / f"{asset}_events.csv"

    def errors_file(self, asset_id: Optional[Any] = None) -> Path:
        asset = self._resolve_asset_id(asset_id)
        return self.base_path / "errors" / f"{asset}_errors.csv"
    ...
    def telemetry_file_for_date(self, date_str: str, asset_id: Optional[Any] = None) -> Path:
        return self.telemetry_dir(asset_id) / f"{self._validate_date_string(date_str)}.csv"
    ...
    def _parse_fields(fields: Optional[Any]) -> Optional[List[str]]:
        ...
        selected = [item.strip() for item in fields_text.split(",") if item.strip()]
    ...
    def _row_matches_search(row: Dict[str, Any], search: Optional[Any]) -> bool:
        ...
        if not search_text:
            return True
        return search_text in " ".join(str(value) for value in row.values()).lower()

    @staticmethod
    def _apply_field_selection(rows: List[Dict[str, Any]], fields: Optional[List[str]]) -> List[Dict[str, Any]]:
        if not fields:
            return rows
    ...
    def _filter_rows(
        ...
        for row in rows:
            ...
            if date_text and self._extract_date(timestamp) != date_text:
                continue
    ...
    def list_assets(self) -> Dict[str, Any]:
        ...
        return {"status": "ok", "base_path": str(self.base_path), "assets_count": len(ordered_assets), "assets": ordered_assets, "default_asset_id": self.asset_id}
    ...
    def get_storage_status(self, asset_id: Optional[Any] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "telemetry_log_bytes": self._directory_size(self.telemetry_dir(resolved_asset_id)),
            "event_log_bytes": self._file_size(self.events_file(resolved_asset_id)),
            "error_log_bytes": self._file_size(self.errors_file(resolved_asset_id)),
    ...
    def list_telemetry_files(self, asset_id: Optional[Any] = None) -> Dict[str, Any]:
        ...
        if not directory.exists():
            return {"status": "error", "asset_id": resolved_asset_id, "message": "Telemetry log directory not found", "directory": str(directory), "files_count": 0, "files": []}
        files = sorted([path.name for path in directory.glob("*.csv")], reverse=True)
        return {"status": "ok", "asset_id": resolved_asset_id, "directory": str(directory), "files_count": len(files), "files": files}
    ...
    def get_telemetry_logs(
        ...
        communication_status: Optional[Any] = None,
        bcu_state: Optional[Any] = None,
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        if not file_path.exists():
            return {"status": "error", "message": "Metadata file not found", "file": str(file_path), "metadata": {}}
        ...
        try:
            ...
            return {"status": "ok", "file": str(file_path), "metadata": metadata}
        except Exception as error:
            return {"status": "error", "message": str(error), "file": str(file_path), "metadata": {}}

    def get_telemetry_csv_download_path(self, date: str, asset_id: Optional[Any] = None) -> Path:
        resolved_asset_id = self._resolve_asset_id(asset_id)
        file_path = self.telemetry_file_for_date(date, asset_id=resolved_asset_id)
        if not file_path.exists():
            raise FileNotFoundError(f"Telemetry CSV not found for asset_id={resolved_asset_id}, date={date}")
        return file_path
