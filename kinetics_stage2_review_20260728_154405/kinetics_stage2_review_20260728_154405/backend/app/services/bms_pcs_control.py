...
class BmsPcsControlService:
    ...
    def status(self, pair_id: str, *, fresh: bool = True) -> dict[str, Any]:
        ...
        blockers = {
            ...
            # Field finding: BAU 0x1001 can assert system_full and system_empty
            # simultaneously during an individually precharged rack. Those
            # global summary bits remain visible below for diagnostics, but
            # pair-level permission is derived from the selected rack's own
            # current limits plus the documented detailed fault words.
        ...
        result = {
            ...
            "summary": {
                # Backward-compatible runtime field: for individual pair
                # operation, a rack is considered enabled when local BCU
                # precharge has succeeded and both required contactors are
                # closed. The central BAU bit is retained separately.
    ...
    def _runtime_monitor_worker(self, username: str, pair: ControlPairConfig) -> None:
        ...
        try:
            while True:
                ...
                if prohibited:
                    violations.append(f"{direction}_prohibited_by_selected_rack_limit_or_detailed_fault")
                if current_limit is None or float(current_limit) < self.config.minimum_bms_current_limit_a:
